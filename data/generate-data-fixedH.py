...
Js_list = np.linspace(0, 1, grid_num)
...
if __name__ == '__main__':
    for ii in range(grid_num):
        for jj in range(grid_num):
            M[ii][jj],Q[ii][jj],sig[ii][jj],S[ii][jj],S_r[ii][jj],sigb[ii][jj] = function(Js_list[ii], beta_list[jj])

    np.save('data-heatmap-H0-'+str(H0)+'.npy', (M, Q, sig, S, S_r, sigb))
